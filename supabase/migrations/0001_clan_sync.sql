...
begin
  ...
  values (p_clan_id, encode(digest(invite_code, 'sha256'), 'hex'), auth.uid(), p_max_uses, p_expires_at)
...
begin
  ...
  where code_hash = encode(digest(upper(trim(coalesce(p_invite_code, ''))), 'sha256'), 'hex')
  for update;
